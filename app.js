...
function toast(t){const e=byId("toast");if(!e)return;e.textContent=t;e.classList.add("show");setTimeout(()=>e.classList.remove("show"),1700)}
function go(id){document.querySelectorAll(".view").forEach(v=>v.classList.remove("active"));const target=byId(id);if(target)target.classList.add("active");document.querySelectorAll(".nav button").forEach(b=>b.classList.toggle("active",b.dataset.go===id))}
document.querySelectorAll("[data-go]").forEach(b=>b.addEventListener("click",()=>go(b.dataset.go)));

function renderHome(){const primeScore=byId("primeScore");if(primeScore)primeScore.textContent=state.score;const elo=byId("elo");if(elo)elo.textContent=state.elo;const rankLabel=byId("rankLabel");if(rankLabel)rankLabel.textContent=state.score<50?"SUB 5":state.score<65?"MTN":state.score<75?"HTN":state.score<85?"LTN":"CHAD"}
...
const goFaceBtn=byId("goFace"),musicTopBtn=byId("musicTop"),menuBtn=byId("menu");
if(goFaceBtn)goFaceBtn.onclick=()=>go("face");if(musicTopBtn)musicTopBtn.onclick=()=>go("music");if(menuBtn)menuBtn.onclick=()=>go("home");
...
async function renderSongs(){const box=byId("songs");if(!box)return;try{tracks=await allTracks();box.innerHTML=tracks.length?tracks.map((t,i)=>`<div class="song"><span>▶</span><b data-i="${i}">${escapeHtml(t.name)}</b><button data-del="${t.id}">×</button></div>`).join(""):`<div class="song">Пока нет треков. Добавь свой первый трек.</div>`;box.querySelectorAll("[data-i]").forEach(b=>b.onclick=()=>playTrack(+b.dataset.i));box.querySelectorAll("[data-del]").forEach(b=>b.onclick=async()=>{try{await delTrack(+b.dataset.del);if(current===+b.dataset.del){audio.pause();audio.removeAttribute("src");current=-1}await renderSongs();toast("Трек удалён")}catch(e){toast(e.message)}})}catch(e){toast("Не удалось загрузить музыку")}}
async function playTrack(i){if(!tracks[i])return;current=tracks[i].id;try{audio.pause();const resp=await fetch(`/api/music/${tracks[i].id}?user_id=${encodeURIComponent(uid)}`);if(!resp.ok)throw new Error("Трек недоступен");const blob=await resp.blob();if(audio._objectUrl)URL.revokeObjectURL(audio._objectUrl);audio._objectUrl=URL.createObjectURL(blob);audio.src=audio._objectUrl;const nowPlaying=byId("nowPlaying");if(nowPlaying)nowPlaying.textContent=tracks[i].name;await audio.play();const playBtn=byId("play");if(playBtn)playBtn.textContent="Ⅱ"}catch(e){toast(e.message||"Не удалось воспроизвести трек")}}
const addMusicBtn=byId("addMusic"),musicInput=byId("musicInput");
if(addMusicBtn&&musicInput)addMusicBtn.onclick=()=>musicInput.click();
if(musicInput)musicInput.onchange=async e=>{const files=[...e.target.files].filter(f=>f.type.startsWith("audio/"));if(!files.length)return;if(addMusicBtn)addMusicBtn.disabled=true;try{for(const f of files){toast(`Загружаю: ${f.name}`);await addTrack(f)}await renderSongs();toast("Треки сохранены в аккаунте")}catch(err){toast(err.message||"Ошибка загрузки")}finally{if(addMusicBtn)addMusicBtn.disabled=false;e.target.value=""}};
const playBtn=byId("play"),prevBtn=byId("prev"),nextBtn=byId("next"),seekInput=byId("seek");
if(playBtn)playBtn.onclick=async()=>{if(!audio.src){if(tracks[0])await playTrack(0);return}if(audio.paused){audio.play();playBtn.textContent="Ⅱ"}else{audio.pause();playBtn.textContent="▶"}};
if(prevBtn)prevBtn.onclick=()=>{if(!tracks.length)return;const i=Math.max(0,tracks.findIndex(t=>t.id===current));playTrack((i-1+tracks.length)%tracks.length)};
if(nextBtn)nextBtn.onclick=()=>{if(!tracks.length)return;const i=Math.max(0,tracks.findIndex(t=>t.id===current));playTrack((i+1)%tracks.length)};
if(seekInput)seekInput.oninput=e=>{if(audio.duration)audio.currentTime=audio.duration*e.target.value/100};audio.ontimeupdate=()=>{if(audio.duration&&seekInput)seekInput.value=audio.currentTime/audio.duration*100};audio.onended=()=>{if(tracks.length){const i=tracks.findIndex(t=>t.id===current);playTrack((i+1)%tracks.length)}};
function escapeHtml(s){return s.replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#039;"}[m]))}
renderSongs();
const resetAdviceBtn=byId("resetAdvice");
if(resetAdviceBtn)resetAdviceBtn.onclick=()=>toast("Принято");
byId("leaderBtn")?.addEventListener("click",leaderboard);
